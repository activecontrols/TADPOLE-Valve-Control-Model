...
mdot_ox_EST = A_th_ox .* sqrt(2 * rho * DPVenturiOX * g ./ (1 - (A_th_ox / A_in)^2));

% Modify when using a diffrent fluid. Change density and can't assume SG of 1.
cv2OX = mdot_ox / rho / 231 * 60 .* sqrt(1 ./ DPValveOX);
cv2IPA = mdot_ipa / rho / 231 * 60 .* sqrt(1 ./ DPValveIPA);

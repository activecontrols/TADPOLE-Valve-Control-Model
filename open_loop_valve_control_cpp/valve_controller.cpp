...
void open_loop_thrust_control(double thrust, sensor_data current_sensor_data, double *angle_ox, double *angle_fuel) {
  double mass_flow_ox;
  double mass_flow_fuel;
  mass_balance(mass_flow_rate(chamber_pressure(thrust)), &mass_flow_ox, &mass_flow_fuel);

  double ox_valve_downstream_pressure_goal = cavitating_venturi(mass_flow_ox, ox_cv, ox_from_temperature(current_sensor_data.ox_cv_temperature));
  double ipa_valve_downstream_pressure_goal = cavitating_venturi(mass_flow_fuel, ipa_cv, ipa_from_temperature(current_sensor_data.ipa_cv_temperature));
...
void open_loop_thrust_control_defaults(double thrust, double *angle_ox, double *angle_fuel) {
  open_loop_thrust_control(thrust, default_sensor_data, angle_ox, angle_fuel);
}

// void update_double_value(double *value, const char *prompt) {
//   Router::info(prompt);
...
//   if (result != 1) {
//     Router::info("Could not convert input to a double, not continuing");
//     return;
//   }
//   *value = new_value;
// }
